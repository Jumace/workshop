...
When creating a new article in a metadata-driven site, write the metadata first, then the body. Treat the metadata title as the page's main headline. The first visible Markdown heading should introduce the opening section, not repeat the title.

## Out of scope

- Mining for new fragments that aren't in the pile (handle gaps as in "Pulling from the pile").
- Editing the raw material file.
- Publishing or formatting for a specific platform beyond the destination file's required article metadata.
